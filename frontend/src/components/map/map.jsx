...
const shipMePositions = [[-20.710,-35.12]];
    ...
    return (
        <> 
            ...
            <MapContainer style={{ height: '100%', width: '100%'}} center={center} zoom={2} minZoom={1} scrollWheelZoom={false} >
                ...
                {
                    sampleMePositions.map(position => <Marker position={position} icon={sampleMeIcon}></Marker> )
                }

                {
                    sampleAllPositions.map(position => <Marker position={position} icon={sampleAllIcon}></Marker> )
                } 

                {
                    shipMePositions.map(position => <Marker position={position} icon={shipMeIcon}></Marker> )
                }
                
                {
                    shipAllPositions.map(position => <Marker position={position} icon={shipAllIcon}></Marker>)
                }

                {
                    homeMePositions.map(position => <Marker position={position} icon={homeMeIcon}></Marker> )
                }

                {
                    homeAllPositions.map(position => <Marker position={position} icon={homeAllIcon}></Marker>)
                }
